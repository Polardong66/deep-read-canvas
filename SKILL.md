...
| HTML 滑块重排后出现显示异常（闪烁后消失/内容不显示） | 🔴 不要多次 Edit 修补——直接 Write 完整重写文件，使用一致的 CSS 类名体系 | 重写后执行 `tail -3` + `present_files` 验证 |
...
**HTML 编辑安全规则**：
- 滑块重排（调整页面顺序）时，不要使用 Python 正则替换 data-idx——直接 Write 完整文件，用 `data-idx` 匹配 DOM 顺序天然一致。
- CSS 类名使用简短无冲突的命名体系（如 `.s` / `.sg` / `.sq`），避免多次 Edit 后类名被意外截断产生残留。
- 显隐机制使用 `display:none` + `.a` 双保险，而非仅 `opacity` 控制。
...
根据书籍的实际章节数量灵活调整，**导读前置**是强制规则——行业定位和读者评价必须在章节内容之前：

```
Slide 01 — 封面
Slide 02 — 全局总览（知识图谱 + 统计卡片）
Slide 03 — 📖 导读①：行业定位（纵向时间线 + 横向同类书对比 + 定位判断）
Slide 04 — 📖 导读②：读者评价（精选2-3条点评，帮助读者建立阅读预期）
Slide 05 — Part I 过渡页
Slide 06-08 — Part I 内容页（每章或合并章节，1-2页）
Slide 09 — Part II 过渡页
Slide 10-11 — Part II 内容页
...
Slide N-2 — 行动建议 / 应用场景页
Slide N-1 — 总结提炼
Slide N   — （备选）引用页
```

**导读双页是固定开头**，无论书籍结构如何，都必须包含。行业交叉分析不再作为收尾页，而是作为引导读者理解本书在该领域坐标的入口。读者评价页提供阅读预期，帮助读者在深入章节前了解「这是一本什么样的书」。

**页码弹性规则**（含固定导读双页）：6篇以上 14-16页，4篇左右 15-17页，无篇结构 12-14页。

**内容深度规则**：
- 每张章节卡片必须包含：① 真实案例（书中原文，含地名/人名/数据）② 逻辑链条（现象→论证→结论）③ 划线人数标注。严禁只有结论没有过程。
- 行业交叉页：纵向时间线 SVG + 横向 5-6 书对比表 + 80 字定位判断。
- 读者评价页：引用真实读者点评原文，标注读者名和星级。
